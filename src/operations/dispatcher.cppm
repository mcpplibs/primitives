...
struct dispatcher_meta {
  ...
  using lhs_traits =
      primitives::traits::primitive_traits<lhs_primitive>;
  using rhs_traits =
      primitives::traits::primitive_traits<rhs_primitive>;
  ...
  using common_rep = policy::type_handler<type_policy, OpTag, lhs_rep,
                                                   rhs_rep>::common_rep;

  static constexpr bool type_ready =
      policy::type_handler_available<type_policy, OpTag, lhs_rep, rhs_rep>;
  static constexpr bool concurrency_ready =
      policy::concurrency_handler_available<concurrency_policy, OpTag,
                                            common_rep, ErrorPayload>;
  static constexpr bool value_ready =
      policy::value_handler_available<value_policy, OpTag, common_rep,
                                      ErrorPayload>;
  static constexpr bool error_ready =
      policy::error_handler_available<error_policy, OpTag, common_rep,
                                      ErrorPayload>;
};
...
constexpr auto dispatch(Lhs const &lhs, Rhs const &rhs)
  ...
  using concurrency_handler_t =
      policy::concurrency_handler<typename meta::concurrency_policy, OpTag,
                                  common_rep, ErrorPayload>;
  using value_handler_t =
      policy::value_handler<typename meta::value_policy, OpTag, common_rep,
                            ErrorPayload>;

  static_assert(
      meta::type_ready,
      "Missing type_handler specialization for this operation/policy");
  static_assert(policy::type_handler<typename meta::type_policy, OpTag,
  ...
  auto const lhs_rep_raw = underlying::traits<
      typename meta::lhs_value_type>::to_rep(lhs.value());
  auto const rhs_rep_raw = underlying::traits<
      typename meta::rhs_value_type>::to_rep(rhs.value());

  if (!underlying::traits<
          typename meta::lhs_value_type>::is_valid_rep(lhs_rep_raw) ||
      !underlying::traits<
          typename meta::rhs_value_type>::is_valid_rep(rhs_rep_raw)) {
    policy::error_request<common_rep> request{};
    request.kind = policy::runtime_error_kind::domain_error;
  ...
  auto const lhs_value_normalized = underlying::traits<
      typename meta::lhs_value_type>::from_rep(lhs_rep_raw);
  auto const rhs_value_normalized = underlying::traits<
      typename meta::rhs_value_type>::from_rep(rhs_rep_raw);
  ...
  auto const rhs_common = static_cast<common_rep>(
      underlying::traits<
          typename meta::rhs_value_type>::to_rep(rhs_value_normalized));
