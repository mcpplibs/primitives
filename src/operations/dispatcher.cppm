...
concept primitive_instance = requires {
  typename mcpplibs::primitives::traits::primitive_traits<
...
struct dispatcher_meta {
  ...
  using lhs_traits =
      mcpplibs::primitives::traits::primitive_traits<lhs_primitive>;
  using rhs_traits =
      mcpplibs::primitives::traits::primitive_traits<rhs_primitive>;

  using lhs_rep = typename lhs_traits::value_type;
  using rhs_rep = typename rhs_traits::value_type;

  using type_policy = typename lhs_traits::type_policy;
  using value_policy = typename lhs_traits::value_policy;
  using error_policy = typename lhs_traits::error_policy;
  using concurrency_policy = typename lhs_traits::concurrency_policy;

  using common_rep = typename policy::type_handler<type_policy, OpTag, lhs_rep,
                                                   rhs_rep>::common_rep;
...
constexpr auto dispatch(Lhs const &lhs, Rhs const &rhs)
  ...
  using common_rep = typename meta::common_rep;
  ...
  auto const lhs_common = static_cast<common_rep>(lhs.value());
  auto const rhs_common = static_cast<common_rep>(rhs.value());
